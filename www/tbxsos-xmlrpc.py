...
# config file path
ini_conf_path = "/etc/teambox/tbxsos-xmlrpc/tbxsos-xmlrpc.ini"
...
def is_in_debug():
    	return True
	return os.environ.has_key("KPSAPIDEV") and str(os.environ["KPSAPIDEV"]) == "1"
...
# This function gets the main key ID.
def main_key_id():
    try:
        k = Key()
        nk = k.fromFile(main_priv_enc_key_path)
        if not nk.id or not str(nk.id).isdigit() or int(nk.id) == 0:
            log.error("main_key_id(): bad key id: '%s'" % ( str(nk.id) ) )
            raise xmlrpclib.Fault(105, "Bad server activation.")
        return int(nk.id)

    except Exception, e:
        log.error("main_key_id(): exception: '%s'" % ( str(e) ) )
        raise xmlrpclib.Fault(105, "Bad server activation.")
...
class KPSApi:
    ...
    # Get the main key of this kps.
    # Returns: (str bigint) key ID
    def get_main_key_id(self, sid):
        # Load session.
        session = session_load(sid)

        return bigint_to_str(session.data["main_key_id"])

    # FIXME: FOR DEV ONLY! TODO: REMOVE!
    # Get the key used for this login.
    # Returns: (str bigint) key ID
    def get_key_id(self, sid):
        # Load session.
        session = session_load(sid)

        return bigint_to_str(session.data["key_id"])
